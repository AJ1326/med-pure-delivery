define('ace/mode/csound_preprocessor_highlight_rules', [
  ...
  var r = e('../lib/oop'),
    ...
    s = function() {
      (this.semicolonComments = { token: 'comment.line.semicolon.csound', regex: ';.*$' }),
        (this.comments = [
          {
            token: 'punctuation.definition.comment.begin.csound',
            regex: '/\\*',
            push: [
              { token: 'punctuation.definition.comment.end.csound', regex: '\\*/', next: 'pop' },
        ...
        (this.macroUses = [
          ...
          { token: 'entity.name.function.preprocessor.csound', regex: /\$[A-Z_a-z]\w*(?:\.|\b)/ }
        ]),
        (this.numbers = [
          {
        ...
        (this.bracedStringContents = [
          { token: 'constant.character.escape.csound', regex: /\\(?:[\\abnrt"]|[0-7]{1,3})/ },
          { token: 'constant.character.placeholder.csound', regex: /%[#0\- +]*\d*(?:\.\d+)?[diuoxXfFeEgGaAcs]/ },
          { token: 'constant.character.escape.csound', regex: /%%/ }
        ]),
        (this.quotedStringContents = [this.macroUses, this.bracedStringContents]);
      var e = [
        this.comments,
        { token: 'keyword.preprocessor.csound', regex: /#(?:e(?:nd(?:if)?|lse)\b|##)|@@?[ \t]*\d+/ },
        {
          token: 'keyword.preprocessor.csound',
          regex: /#include/,
          push: [this.comments, { token: 'string.csound', regex: /([^ \t])(?:.*?\1)/, next: 'pop' }]
        },
        { token: 'keyword.preprocessor.csound', regex: /#[ \t]*define/, next: 'define directive' },
        { token: 'keyword.preprocessor.csound', regex: /#(?:ifn?def|undef)\b/, next: 'macro directive' },
        this.macroUses
      ];
      this.$rules = {
        start: e,
        'define directive': [
          this.comments,
          { token: 'entity.name.function.preprocessor.csound', regex: /[A-Z_a-z]\w*/ },
          {
        ...
        'macro parameter name list': [
          { token: 'variable.parameter.preprocessor.csound', regex: /[A-Z_a-z]\w*/ },
          {
        ...
        'macro body': [
          { token: 'constant.character.escape.csound', regex: /\\#/ },
          { token: 'punctuation.definition.macro.end.csound', regex: /#/, next: 'start' },
          e
        ],
        'macro directive': [
          this.comments,
          { token: 'entity.name.function.preprocessor.csound', regex: /[A-Z_a-z]\w*/, next: 'start' }
        ],
        'macro parameter value list': [
          { token: 'punctuation.definition.macro-parameter-value-list.end.csound', regex: /\)/, next: 'start' },
          {
        ...
        'macro parameter value quoted string': [
          ...
          { token: 'punctuation.definition.string.end.csound', regex: /"/, next: 'macro parameter value list' },
          this.quotedStringContents,
          { defaultToken: 'string.quoted.csound' }
        ],
        'macro parameter value parenthetical': [
          { token: 'constant.character.escape.csound', regex: /\\\)/ },
          this.popRule({ token: 'punctuation.macro-parameter-value-parenthetical.end.csound', regex: /\)/ }),
          this.pushRule({
            token: 'punctuation.macro-parameter-value-parenthetical.begin.csound',
            regex: /\(/,
            next: 'macro parameter value parenthetical'
      ...
      (this.pushRule = function(e) {
        return {
          ...
          onMatch: function(t, n, r, i) {
            r.length === 0 && r.push(n);
            if (Array.isArray(e.next)) for (var s = 0; s < e.next.length; s++) r.push(e.next[s]);
            else r.push(e.next);
              ...
              if (i > r)
                while (r >= 0 && i >= 0) {
                  if (n.charAt(r) !== t.charAt(i)) {
                    var s = t.substr(0, i);
                    for (var o = 0; o < e.next.length; o++) e.next[o] = s + e.next[o];
                    break;
                  }
                  r--, i--;
                }
  ...
  define('ace/mode/csound_score_highlight_rules', [
    ...
    var r = e('../lib/oop'),
      ...
      s = function() {
        i.call(this), this.quotedStringContents.push({ token: 'invalid.illegal.csound-score', regex: /[^"]*$/ });
        ...
        e.push(
          ...
          },
          {
            ...
            push: [
              ...
              this.comments,
              { token: 'entity.name.label.csound-score', regex: /[A-Z_a-z]\w*/ }
            ]
          },
          { token: 'keyword.preprocessor.csound-score', regex: /r\b/, next: 'repeat section' },
          this.numbers,
          { token: 'keyword.operator.csound-score', regex: '[!+\\-*/^%&|<>#~.]' },
          this.pushRule({
          ...
          this.addRules({
            ...
            'repeat section before label': [
              { token: 'empty', regex: /$/, next: 'start' },
              this.comments,
              { token: 'entity.name.label.csound-score', regex: /[A-Z_a-z]\w*/, next: 'start' }
            ],
            'quoted string': [
              this.popRule({ token: 'punctuation.definition.string.end.csound-score', regex: /"/ }),
              this.quotedStringContents,
            ...
            'loop after macro name': [
              e,
              this.popRule({ token: 'punctuation.braced-loop.end.csound-score', regex: /}/ })
            ]
          }),
          this.normalizeRules();
      };
              ...
              next: [
                {
                  onMatch: function(e, t, n) {
                    return (
                      e.length == n[1] ? (n.shift(), n.shift(), (this.next = n.shift())) : (this.next = ''),
                      'string.end'
                    );
                  },
                  regex: /\]=*\]/,
      ...
      f = function() {
        ...
        (e = r.arrayToMap(e)),
          ...
          (this.lineContinuations = [
            { token: 'constant.character.escape.line-continuation.csound', regex: /\\$/ },
            this.pushRule({
          ...
          this.quotedStringContents.push(this.lineContinuations, { token: 'invalid.illegal', regex: /[^"\\]*$/ });
        ...
        n.splice(1, 0, {
          ...
          n.push(
            ...
            this.pushRule({ token: 'keyword.function.csound', regex: /\bopcode\b/, next: 'after opcode keyword' }),
            { token: 'keyword.other.csound', regex: /\bend(?:in|op)\b/ },
            { token: 'variable.language.csound', regex: /\b(?:0dbfs|A4|k(?:r|smps)|nchnls(?:_i)?|sr)\b/ },
            this.numbers,
            {
              token: 'keyword.operator.csound',
              regex: '\\+=|-=|\\*=|/=|<<|>>|<=|>=|==|!=|&&|\\|\\||[~\u00ac]|[=!+\\-*/^%&|<>#?:]'
            },
            this.pushRule({ token: 'punctuation.definition.string.begin.csound', regex: /"/, next: 'quoted string' }),
            this.pushRule({ token: 'punctuation.definition.string.begin.csound', regex: /{{/, next: 'braced string' }),
            {
              token: 'keyword.control.csound',
              regex: /\b(?:do|else(?:if)?|end(?:if|until)|fi|i(?:f|then)|kthen|od|r(?:ir)?eturn|then|until|while)\b/
            },
            this.pushRule({ token: 'keyword.control.csound', regex: /\b[ik]?goto\b/, next: 'goto before label' }),
            this.pushRule({
              token: 'keyword.control.csound',
            ...
            this.pushRule({ token: 'support.function.csound', regex: /\bpyl?run[it]?\b(?!$)/, next: 'Python opcode' }),
            this.pushRule({
              token: 'support.function.csound',
              ...
              onMatch: function(n, r, i, s) {
                ...
                return (
                  e.hasOwnProperty(u)
                    ...
                    : t.hasOwnProperty(u) && (a = 'invalid.deprecated.csound'),
                  a
                    ? o[2]
                      ? [
                          { type: a, value: u },
                          { type: 'punctuation.type-annotation.csound', value: o[2] },
                          { type: 'type-annotation.storage.type.csound', value: o[3] }
                        ]
                      : a
                    : 'text.csound'
                );
          ...
          this.addRules({
            ...
            'instrument numbers and identifiers': [
              this.comments,
              { token: 'entity.name.function.csound', regex: /\d+|[A-Z_a-z]\w*/ },
            ...
            'quoted string': [
              this.popRule({ token: 'punctuation.definition.string.end.csound', regex: /"/ }),
              this.quotedStringContents,
              { defaultToken: 'string.quoted.csound' }
            ],
            'braced string': [
            ...
            'goto before argument': [this.popRule({ token: 'text.csound', regex: /,/ }), n],
            'goto before label': [
            ...
            'Csound score opcode': [
              this.comments,
              { token: 'punctuation.definition.string.begin.csound', regex: /{{/, next: 'csound-score-start' },
              this.popRule({ token: 'empty', regex: /$/ })
            ],
            'Python opcode': [
              this.comments,
              { token: 'punctuation.definition.string.begin.csound', regex: /{{/, next: 'python-start' },
              this.popRule({ token: 'empty', regex: /$/ })
            ],
            'Lua opcode': [
              this.comments,
              { token: 'punctuation.definition.string.begin.csound', regex: /{{/, next: 'lua-start' },
              this.popRule({ token: 'empty', regex: /$/ })
            ],
            'line continuation': [
              this.popRule({ token: 'empty', regex: /$/ }),
        ...
        var i = [this.popRule({ token: 'punctuation.definition.string.end.csound', regex: /}}/ })];
        ...
        (this.$rules = {
          ...
          ruleset: [
            ...
            { include: ['strings', 'url', 'comments'] },
            {
              ...
              regex:
                '(' + c + ')(ch|cm|deg|em|ex|fr|gd|grad|Hz|in|kHz|mm|ms|pc|pt|px|rad|rem|s|turn|vh|vmax|vmin|vm|vw|%)'
            },
            { token: 'constant.numeric', regex: c },
