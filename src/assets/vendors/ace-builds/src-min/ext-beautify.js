        ...
        if (typeof s != 'undefined') {
          ...
          if (d) {
            s.type === 'keyword' && d.match(/^(if|else|elseif|for|foreach|while|switch)$/)
              ? ((P[m] = d), H(), (h = !0), d.match(/^(else|elseif)$/) && p.match(/\}[\s]*$/) && (j(), (c = !0)))
              : s.type === 'paren.lparen'
              ? (H(),
              ...
              ? ((w = 1),
                d.substr(0, 1) === '}' &&
                  (P[m - 1] === 'case' && w++, p.trimRight().substr(-1) === '{' ? j() : ((c = !0), M && (N += 2))),
                d.substr(0, 1) === ']' &&
                  p.substr(-1) !== '}' &&
                  p.trimRight().substr(-1) === '}' &&
                  ((c = !1), b++, j()),
            ...
            s.type === 'keyword' && d.match(/^(case|default)$/) && ((P[m] = d), m++),
              s.type === 'keyword' && d.match(/^(break)$/) && P[m - 1] && P[m - 1].match(/^(case|default)$/) && m--,
              s.type === 'paren.lparen' && ((E += (d.match(/\(/g) || []).length), (m += d.length)),
