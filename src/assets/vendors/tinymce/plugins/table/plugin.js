...
jsc*/
   ...
   * @private
   */
  define('tinymce.plugins.table.util.Utils', ['tinymce.core.Env'], function(Env) {
   ...
   * @private
   */
  define('tinymce.plugins.table.ui.Dialogs', ['tinymce.core.util.Tools', 'tinymce.core.Env'], function(Tools, Env) {
    var each = Tools.each;
      ...
      self.table = function(isProps) {
          ...
          if (tableElm) {
            data = {
              ...
              cellspacing: removePxSuffix(
                dom.getStyle(tableElm, 'border-spacing') || dom.getAttrib(tableElm, 'cellspacing')
              ),
              cellpadding:
        ...
        if (editor.settings.table_class_list) {
          ...
          classListCtrl = {
            ...
            type: 'listbox',
            label: 'Class',
            values: buildListItems(editor.settings.table_class_list, function(item) {
              if (item.value) {
          ...
          items: [
            {
              ...
              items:
                editor.settings.table_appearance_options !== false
                  ...
                  : [colsCtrl, rowsCtrl, { label: 'Width', name: 'width' }, { label: 'Height', name: 'height' }]
            },
      ...
      self.cell = function() {
        ...
        if (editor.settings.table_cell_class_list) {
          classListCtrl = {
            ...
            type: 'listbox',
            label: 'Class',
            values: buildListItems(editor.settings.table_cell_class_list, function(item) {
              if (item.value) {
                item.textStyle = function() {
      ...
      self.row = function() {
        ...
        if (editor.settings.table_row_class_list) {
          classListCtrl = {
            ...
            type: 'listbox',
            label: 'Class',
            values: buildListItems(editor.settings.table_row_class_list, function(item) {
              if (item.value) {
                item.textStyle = function() {
   ...
   * @private
   */
  define('tinymce.plugins.table.ui.ResizeBars', ['tinymce.core.util.Tools', 'tinymce.core.util.VK'], function(
    Tools,
    VK
  ) {
    var hoverTable;
    ...
    return function(editor) {
      ...
      function adjustWidth(table, delta, index) {
        ...
        var percentageBased = isPercentageBasedSize(table.width) || isPercentageBasedSize(table.style.width);
        ...
        var newTableWidth = percentageBased ? getNewTablePercentWidth() : getNewTablePixelWidth();
    ...
    return function(editor) {
      ...
      function fixBeforeTableCaretBug() {
        ...
        function isAtStart(rng, par) {
          ...
          return (
            elm.innerHTML.replace(/<(br|img|object|embed|input|textarea)[^>]*>/gi, '-').replace(/<[^>]+>/g, '')
              .length === 0
          );
        }
      ...
      function handleDeleteInCaption() {
        ...
        var contractSelectionTo = function(caption) {
          ...
          if (rng.commonAncestorContainer === caption.parentNode && isTheHeirOf(rng.startContainer, caption)) {
        ...
        editor.on('keydown', function(e) {
          ...
          if (!editor.selection.isCollapsed()) {
            ...
            if (isEmptyNode(container) || (e.keyCode === VK.BACKSPACE && caretIsAtTheLeftEdgeOf(container))) {
              // see TINY-979
              e.preventDefault();
            }
          }
  ...
  define('tinymce.plugins.table.Plugin', [
    ...
    function Plugin(editor) {
      ...
      editor.on('PreInit', function() {
        ...
        editor.serializer.addAttributeFilter('data-mce-cell-padding,data-mce-border,data-mce-border-color', function(
          nodes,
          name
        ) {
          var i = nodes.length;
